...
int Shell_exec(Shell template, ...)
{
	...
	check(changes == template.num_changes, "Invalided arguments.");
...
int Shell_run(apr_pool_t *p, Shell *cmd)
{
	...
	rv = apr_proc_create(&newsproc, cmd->exe, cmd->args, NULL, attr, p);
	check(rv == APR_SUCCESS, "Failed to run command.");

	rv = apr_proc_wait(&newproc, &cmb->exit_code, &cmd->exit_why, APR_WAIT);
