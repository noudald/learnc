...
int Command_fetch(apr_pool_t *p, const char *url, int fetch_only)
{
	...
	} else if (apr_fnmatch(DEPND_PAT, info.path, 0) == APR_SUCCESS) {
	...
	const char *make_opts, const char *install_opts)
{
	...
	if (access(CONFIG_SCRIPT, X_OK) == 0) {
		log_info("Has a configure script, running it now.");
		rc = Shell_exec(CONFIGURE_SH, "OPTS", configure_opts, NULL);
		check(rc == 0, "Failed to configure");
	}

	rc = Shell_exec(MAKE_SH, "OPTS", make_optss, NULL);
...
inf Command_install(apr_pool_t *p, const char *url, const char *configure_opts,
	const char *make_opts, const char *install_opts)
{
	int rc = 0;
	...
	if (rc == 1) {
		rc = Command_build(p, url, configure_opts, 
			make_opts, install_opts);
		check(rc == 0, "Failed to build: %s", url);
	} else if (rc == 0) {
		log_info("Depends sucessfully insatlled: %s", url);
	} else {
